...
#'
#' @examples
#' \donttest{atlas.status(atlas.connect('http://localhost:8080'))}
#'
#'
...
#'
#' @examples
#' \donttest{atlas.contains(atlas.connect('http://localhost:8080'), 123)}
#'
#'
atlas.contains <- function(connection, patient_id) {
...
#'
#' @examples
#' \donttest{atlas.dump(atlas.connect('http://localhost:8080'), 123, '/path/to/dump/files/')}
#' \donttest{atlas.dump(atlas.connect('http://localhost:8080'), 123, '/path/', 'ICD9=250.50', TRUE, TRUE)}
#'
#'
atlas.dump <- function(connection, patient_id, path, selection_query=NULL, contains_start=FALSE, contains_end=FALSE) {
  ...
  if (atlas.contains(connection, patient_id) == FALSE) {
